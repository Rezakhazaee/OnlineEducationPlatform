...
public class CoursePartnerOrganizationsController : ControllerBase
{
    ...
    public CoursePartnerOrganizationsController(
    ...
    public async Task<ActionResult<IEnumerable<CoursePartnerOrganization>>> GetByCourse(
        int courseId)
    {
    ...
    [HttpPost]
    public async Task<ActionResult<CoursePartnerOrganization>> Create(
        CreateCoursePartnerOrganizationDto request)
    {
        if (!await _packageAccess.HasPackageAsync(4))
        {
            return StatusCode(
        ...
        var organization = await _context.PartnerOrganizations
            .FirstOrDefaultAsync(o =>
                o.Id == request.PartnerOrganizationId);
        ...
        if (duplicate)
        {
            return Conflict(new
            {
                message = "این سازمان قبلاً برای این دوره ثبت شده است."
            });
        }
    ...
    [HttpPut("{id}")]
    public async Task<ActionResult<CoursePartnerOrganization>> Update(
        int id,
        UpdateCoursePartnerOrganizationDto request)
    {
        if (!await _packageAccess.HasPackageAsync(4))
        {
            return StatusCode(
        ...
        if (item == null)
        {
            return NotFound(new
            {
                message = "ارتباط دوره و سازمان پیدا نشد."
            });
        }
    ...
    public async Task<IActionResult> Delete(int id)
    {
        ...
        var hasEnrollment = await _context.Enrollments
            .AnyAsync(e =>
                e.CoursePartnerOrganizationId == id);
        ...
        if (hasEnrollment)
        {
            ...
            {
                message =
                    "این ارتباط دارای ثبت‌نام است و قابل حذف نیست. ابتدا آن را غیرفعال کنید."
            });
        }
